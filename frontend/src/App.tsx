...
const [method, setMethod] = useState<"basic" | "ttest" | "correlation" | "regression" | "multiple_regression" | "anova">("basic");
  ...
  try {
    const response = await fetch("http://localhost:8080/api/analyses", {
      ...
      body: JSON.stringify({
        ...
        options:
  method === "ttest" ? { paired } :
  method === "multiple_regression" ? { yColumnIndex } :
  {},
      }),
    });
      ...
      <div className="result-section">
        ...
        {result.method === "ttest" && (
          <div className="stat-summary">
            <div>
              t統計量: {result.result.statistic.toFixed(3)} ／
...
{result.method === "multiple_regression" && (
  <div>
    <div className="stat-summary">
      <div>切片: {result.result.intercept.toFixed(3)} (p={result.result.interceptPValue.toFixed(4)})</div>
    ...
    <div className="graph-row">
      ...
      />
        ...
        {(result.method === "basic" || result.method === "ttest" || result.method === "anova") && result.graphs && (
          <div className="graph-row">
            <img
